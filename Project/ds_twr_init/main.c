...
static dwt_config_t config =
{
    ...
    (257 + 64 - 32) /* SFD timeout (preamble length + 1 + SFD length - PAC size). Used in RX only. */
};
#if 1 //To fold easily.
...
#define TX_DELAY_MS 2/* Loop forever sending frames periodically. */
void Simple_Tx(void)
{
		...
		//dwt_write32bitreg(0x1E,0xFFFFFFFF);//smallest Tx Power
		dwt_write32bitreg(0x1E,0x1F1F1F1F);//max Tx Power user_manual_2.10Page105
#ifdef UART_TX
		while(1)
		{
			{
...
//        dwt_writetxdata(num+2, (u8*)receive_data, 0);
//        dwt_writetxfctrl(num+2, 0);
			}
	  ...
	  while(1)
    {
        ...
        tx_msg[BLINK_FRAME_SN_IDX]++;
				if(tx_msg[BLINK_FRAME_SN_IDX] >= 100)
				{
					tx_msg[BLINK_FRAME_SN_IDX]=0;
				}
				num = tx_msg[BLINK_FRAME_SN_IDX];
				//itoa(tx_msg[1],tx_show,10);
				sprintf(tx_show, "%d",num);
				OLED_ShowString(0,2,(u8*)tx_show);
    }
#endif
}
#endif
...
/* Hold copy of status register state here for reference, so reader can examine it at a breakpoint. */
//static uint32 status_reg = 0;
int num = 0,num_last = 0,num_count = 0;
char rx_show[20];
...
{
		int i;
		OLED_ShowString(0,0,(u8*)"SIMPLE_RX");
    ...
    while (1)
    {
				...
				static int RxC,RxN,RxNOSAT;
				static u8 isDisplay = 0;
			  static double RSSI;
				static uint64 tx_time;
...
//								}
							  ...
							  num = rx_buffer[1];
								num_count++;
								sprintf(rx_show,"%d",num);
								OLED_ShowString(0,2,(u8*)rx_show);
							  if(num < num_last)
								{
									if(isDisplay != 0)
									{
										isDisplay = 0;//second don't display
										num_count = 0;
									}
									else 
									{
										isDisplay = 1;//have displayed once
										sprintf(rx_show,"%f",(float)num_count/100.0f);
										num_count = 0;//clear reset
										OLED_ShowString(0,4,(u8*)"                  ");
										OLED_ShowString(0,4,(u8*)rx_show);
										printf((const char*)rx_show);
										RxC = dwt_read32bitoffsetreg(0x12,0x04);
										RxC = RxC >> 16;
										RxNOSAT = (int)dwt_read16bitoffsetreg(0x27,0x2C);
										RxN = dwt_read32bitoffsetreg(0x10,0x00);
										RxN = RxN >> 20;
										if(RxNOSAT == RxN)
										{
											RxN -= 82;//user_manual_2.10Page94 adjust RXPACC count for SFD
										}
										RSSI= 10*log10(((double)RxC * 131072) / ((double)RxN * (double)RxN))-121.74;
	//									sprintf(rx_show,"%d+%d",RxC,RxN);
	//									OLED_ShowString(0,4,(u8*)rx_show);
										sprintf(rx_show,"RSSI:%lf",RSSI);
										OLED_ShowString(0,6,(u8*)rx_show);
										printf((const char*)rx_show);
										//OLED_ShowString(0,8,&rx_buffer[2]);
...
int main(void)
{
    ...
    if(dwt_initialise(DWT_LOADUCODE) == -1)
    {
        ...
        OLED_ShowString(0,0,(u8*)"INIT FAIL");
        while (1)
        {
            STM_EVAL_LEDOn(LED1);
            deca_sleep(100);
    ...
    OLED_ShowString(0,0,(u8*)"INIT PASS");
    ...
    *ts = 0;
    for (i = 0; i < FINAL_MSG_TS_LEN; i++)
    {
				...
				else
				{
					switch(i)
					{
						case 4: *ts += ((uint64)ts_field[i]) * 0x100000000;break;
						case 5:	*ts += ((uint64)ts_field[i]) * 0x1000000000;break;
						case 6:	*ts += ((uint64)ts_field[i]) * 0x10000000000;break;
						default: break;
					}
				}
    }
